...
for i in range(1):
    ...
    vecs = vecs[np.arange(0, len(vecs), angluar_sub_sampling)]
    ...
    for i in range(len(projs_idx)):
        ...
        a = imageio.imread(os.path.join(data_path_full, f"scan_{projs_idx[i]:06d}.tif"))
        projs[i]=(a-a.min())/(a.max()-a.min()).astype(np.float32)
    ...
    for i in range(200):
        ...
        imageio.imsave(os.path.join(recon_path, f"fdk_{(i+1):06d}.png"), a)
    print(np.round_(time.time() - t, 3), 'sec elapsed')
