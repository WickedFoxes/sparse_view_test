...
model = UNet_2D(args.model.lower() , 
    ...
    with torch.no_grad():
        for walnutId in range(0,1): #33-38
            ...
            for idx in range(63):
                ...
                print(pairData[projId])
                for k in range(4):
                    index=int(pairData[projId,k+1])
                    # img=imageio.imread(dataPath+str(index)+'.tif')
                    img=imageio.imread(os.path.join(dataPath, f"scan_{index:06d}.tif"))
                ...
                for j in range(4):
                    ...
                    print(index, output[j,0,:,:].min(), output[j,0,:,:].max())
                    # imageio.imsave(saveDir+str(index)+'.tif',output[j,0,:,:])
                    imageio.imsave(os.path.join(saveDir, f"scan_{index:06d}.tif"), output[j,0,:,:])
