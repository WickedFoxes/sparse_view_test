...
class MultiScaleFlow(nn.Module):
    def __init__(self, backbone, **kargs):
        ...
        if self.data_chans == 1:
            ...
            raise ValueError(f"Unsupported data_chans: {self.data_chans}")
        self.block = nn.ModuleList([Head( kargs['motion_dims'][-1-i] * kargs['depths'][-1-i] + kargs['embed_dims'][-1-i], 
                            kargs['scales'][-1-i], 
                            kargs['hidden_dims'][-1-i],
                            head_in_else[0] if i == 0 else head_in_else[1]
                            )  # if color (2, 9)-> (6, 17), if gray (6, 17) -> (2, 9)
                            for i in range(self.flow_num_stage)])
        self.unet = Unet(kargs['c'] * 2, out=self.data_chans)
    ...
    def calculate_flow(self, imgs, timestep, af=None, mf=None):
        img0, img1 = imgs[:, :self.data_chans ], imgs[:, self.data_chans :]
        B = img0.size(0)
        flow, mask = None, None
